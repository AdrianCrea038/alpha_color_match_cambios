...
%%EOF`;
    ...
    exportEPS() {
        ...
        this.exportEPSFromColors(colors, plotterValue);
    }
    ...
    renderPreview() {
        ...
        if (colors.length === 0) {
            this.previewContainer.innerHTML = `<div class="empty-state"><div class="empty-icon">🎨</div><p>No hay colores pendientes para mostrar</p></div>`;
            return;
        }
        ...
        this.previewContainer.innerHTML = rows.map(row => {
            return `<div style="display: flex; gap: ${margin}px; justify-content: flex-start;">
                ${row.map(color => {
                    const rgb = this.cmykToRgb(color.cmyk.c, color.cmyk.m, color.cmyk.y, color.cmyk.k);
                    const spotName = `${color.name} ${color.nk}`.toUpperCase();
                    return `<div class="eps-preview-box" style="width: ${boxSize}px; height: ${boxSize}px; background: ${rgb}; border: 1px solid #4b5563; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 9px; color: white; text-shadow: 0 0 2px black; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; padding: 4px; flex-shrink: 0;">
                        <div style="text-align: left; font-weight: bold;">Plotter: ${plotterValue}</div>
                        <div style="text-align: right;">C:${color.cmyk.c.toFixed(0)}<br>M:${color.cmyk.m.toFixed(0)}<br>Y:${color.cmyk.y.toFixed(0)}<br>K:${color.cmyk.k.toFixed(0)}</div>
        ...
        if (statsDiv) {
            statsDiv.innerHTML = `<strong>📊 Resumen EPS:</strong> ${colors.length} colores pendientes | Plotter: ${plotterValue}`;
        }
    }
